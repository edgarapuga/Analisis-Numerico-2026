...
author: "Edgar Alejandro Puga Leguizamo"
format: 
  html:
    grid:
      body-width: 1200px
...
f(x) = x\^2 - 5x + 6 #grafica de la función
...
plt.xlabel("x")
plt.ylabel("f(x)")
plt.title("Gráfica de f(x) = x^2 - 7x + 2.5")
plt.grid()
plt.show()
```
...
r1_new, info_new1 = optimize.newton(
    f, 
    x0=0.7,          # valor inicial menos trivial
    tol=1e-3,
    maxiter=50,
    full_output=True
)
...
r2_new, info_new2 = optimize.newton(
    f, 
    x0=5.7,          # valor inicial más alejado
    tol=1e-3,
    maxiter=50,
    full_output=True
)
...
f(x) = x\^3 - 6x\^2 + 11x - 6 grafica de la función
...
plt.xlabel("x")
plt.ylabel("f(x)")
plt.title("Gráfica de f(x) = x^3 - 3x^2 - 6.5x - 3")
plt.grid()
plt.show()
```

#aproximacion de las raíces bisección
...
f(x) = x\^3 - 5x^2e^{-x} + e\^{-3x}, 0\<= x \<= 2 grafica de la función
...
plt.xlabel("x")
plt.ylabel("f(x)")
plt.title("Gráfica de f(x) = x^3 + 2x^2 e^{-x} + e^{-5x} en [0,2]")
plt.grid()
plt.show()
```
...
g(x)=e\^-x - cos(ax), a=1,3,5,9 grafica de la función
...
::::::: {#exr-tarea1_6}
Todas las raíces de $f(x)= \sqrt{x}-sen(x) -c$ para $c=1,\pi$. ::/ f(x)=sqrt(x) - sen(x) - c, c=1,pi grafica de la función
...
{sen(x) + y\^2 - 1 = 0, x + cos(y) - 1 = 0} Grafica de las curvas de nivel
...
{x\^3 + y - 6 = 0, y\^3 - x - 4 = 0} Grafica de las curvas de nivel
...
curva de demanda: q_d = 200 - 10 p\^2 curva de oferta: q_s = 5 p\^3 + 15 equilibrio: q + 10p\^2 -200 = 0, q - 5p\^3 - 15 = 0 {q + 10p\^2 -200 = 0, q - 5p\^3 - 15 = 0} \# grafica de las curvas
